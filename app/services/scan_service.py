...
def filter_scans(user_id: str, date_from, date_to) -> list:
    """Return scans for a user filtered by an optional date range."""
    if not supabase:
        return []
...
def parse_report_dates(user, date_from_str: str, date_to_str: str):
    """
    Parse and validate report date params.
    Returns (date_from, date_to, error_str | None).
